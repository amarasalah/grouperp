// BC Achat Page - individual product, no qty (each poteau = 1)
...
export async function renderBcAchat() {
  ...
  function openForm(bc = null) {
    ...
    wireAll();
    if (bc?.lignes) { setTimeout(() => { document.querySelectorAll('#bca-lines .line-row').forEach((row, i) => { const l = bc.lignes[i]; if (l?.categorieId) { row.querySelector('.line-cat').value = l.categorieId; populateProducts(row, l.categorieId, l.produitId); } }); recalcAll(); }, 100); }
  ...
  function collectLines() {
    const lines = []; document.querySelectorAll('#bca-lines .line-row').forEach(row => {
