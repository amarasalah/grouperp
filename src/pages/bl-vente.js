// BL Vente Page - individual product, no qty (each poteau = 1)
...
export async function renderBlVente() {
    ...
    function render(filters = currentFilters, page = currentPage) {
        ...
        content.innerHTML = `
      <div class="page-header"><div><h1 class="page-title">🚛 Bon de Livraison Vente</h1><p class="page-subtitle">Livraison clients</p></div>
      ...
      <div id="pagination-controls" class="pagination-container"></div>`;
        ...
        document.querySelectorAll('.validate-blv').forEach(b => b.onclick = async () => {
            ...
            if (bl && await confirmDialog('Valider la livraison ? Stock sera décrémenté.')) {
                try {
                    for (const l of (bl.lignes || [])) {
                        if (l.produitId) await updateStock(l.produitId, 1, 'sortie', bl.numero);
                    }
    ...
    function openForm() {
      ...
      <h4 style="margin:16px 0 8px">Poteaux</h4><div class="table-wrapper"><table class="data-table line-items-table"><thead><tr><th>Catégorie</th><th>Poteau (ID)</th><th></th></tr></thead><tbody id="blv-lines">${lineHtml()}</tbody></table></div>
      ...
      <div class="form-group" style="margin-top:12px"><label class="form-label">Note</label><textarea class="form-input" name="note" rows="2"></textarea></div>
        ...
        document.getElementById('save-blv').onclick = async () => {
            ...
            const cli = clients.find(x => x.id === cid), dt = f.querySelector('[name="date"]').value, lines = collectLines();
            ...
            const id = await add('bl_vente', { numero: n, date: dt, clientId: cid, clientNom: cli?.raisonSociale || '', lignes: lines, statut: 'En attente', note });
            bls.unshift({ id, numero: n, date: dt, clientId: cid, clientNom: cli?.raisonSociale || '', lignes: lines, statut: 'En attente', note });
    ...
    function collectLines() {
        const lines = []; document.querySelectorAll('#blv-lines .line-row').forEach(row => {
