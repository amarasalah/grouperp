...
export async function renderDevisVente() {
  ...
  async function openForm(dv = null) {
    ...
    document.getElementById('save-dv').onclick = async () => {
      ...
      const data = { date: dt, clientId: cid, clientNom: cl?.raisonSociale || '', lignes: lines, taxRate: tax, totalHT, totalTVA, totalTTC: totalHT + totalTVA, statut: dv?.statut || 'Brouillon', note: f.querySelector('[name="note"]').value };
    ...
    function collectLines() {
      const lines = [], seen = new Set(); document.querySelectorAll('#dv-lines .line-row').forEach(row => {
